...
export class ServiceLib extends FileLib {
  static pathDAL = "src/dataaccess/services/";
  ...
  static createInterfaceServiceDAL(className) {
    ...
    const fileName = `${formatFileName(className)}.service.interface.ts`;
    this.createFile(
      this.pathDAL + formatFileFolder(className) + "/",
      fileName,
      str
    );
    console.log(
  ...
  static createServiceFile(className, content, basePath) {
    const dirPath = basePath + formatFileFolder(className) + "/";
